...
add('HTTP redaction', 'safe HTTP', "          void response.body?.cancel().catch(() => {});\n          throw new AiProviderError('http_error', response.status);", "          throw Object.assign(new AiProviderError('http_error', response.status), { body: await response.text() });")
