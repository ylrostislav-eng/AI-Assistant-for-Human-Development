...
Клиент синхронизирует **команды и подтверждённые изменения**, а не произвольные копии таблиц. PostgreSQL определяет confirmed state; SQLite хранит confirmed projection + pending overlay. UI продолжает работать без сети, честно показывая неподтверждённые изменения.
...
Envelope проверяет транспорт. `payload` обязательно отдельно проверяется закрытой схемой для `kind` из registry; schema envelope сама по себе не разрешает свободные поля. Полные OpenAPI/command schemas создаются в P1-01 до route implementation. Черновик здесь не объявляется уже готовым HTTP-сервисом.
...
| Route | Назначение |
|---|---|
| POST /auth/apple | Проверка Apple credential + device registration |
...
| POST /voice/sessions | Phase 4: bounded session authorization |
| POST /devices/push-token | Device-scoped token registration |
...
| GET /health/live; GET /health/ready | Минимальная служебная диагностика без секретов |
...
| 409 idempotency_key_reused | Та же command_id с другим payload; ошибка клиента |
...
2. Одной SQLite-транзакцией записать pending command, optimistic overlay и local notification changes.
3. UI сразу показывает результат; reward estimate — pending, если нужен backend.
4. Sync запускается при foreground, восстановлении сети, successful auth, background opportunity и ручном refresh.
...
Доступно offline: Today/calendar/character/skills/history из кэша; создание/редактирование ручных goals/tasks/events; completion/partial/excuse/reschedule; timer; inbox; локальные reminders. Новая AI-программа/voice требует сети. Архивация возможна локально, account deletion требует сервера и re-auth.

При первом запуске совсем без сети — гостевой локальный draft/onboarding/manual plan без подтверждённых backend rewards. После регистрации явное принятие локальных данных в один account; не смешивать с другим существующим account без import review. Гостевой путь допустимо включить позднее, но экран отсутствия сети и сохранение onboarding обязательны в Phase 1.
...
3. Найти `(user_id, command_id)`. Если hash совпадает, вернуть сохранённый receipt без повторной версии/XP. Если hash другой — reject.
...
| Timer + HealthKit доказали одну тренировку | Attach evidence к root; максимум небольшой delta evidence bonus, без второй duration |
...
SQLite migrations: последовательные, transactional где возможно, до открытия repository. Перед risk migration — encrypted OS-protected local backup; проверка schema/counts; откат при failure. SQL migrations backend — expand → deploy compatible code → backfill → contract, не drop обязательных колонок одновременно с обновлением сервера.
