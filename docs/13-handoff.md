...
**T-02b — обработка принятых обновлений и первые ответы бота.** Приём уже
сделан (T-02a, миграция 015): обновления сохраняются, дедуплицируются и ждут в
`telegram_updates` с `processed_at IS NULL`.

Что входит:

- Обработчик в worker: берёт необработанные обновления по порядку, сопоставляет
  `sender_telegram_id` с аккаунтом через `identity_resolve_telegram`, помечает
  `processed_at`. Отправитель из чужого Telegram (не в allowlist) обрабатывается
  явно — молчание выглядит как поломка.
- Команды `/start` и `/today`. `/today` читает серверный snapshot, а **не**
  разбирает предыдущие сообщения.
- Отправка через Bot API с delivery ledger: таймаут запроса может означать уже
  доставленное сообщение, поэтому нужен отдельный учёт исходов
  `pending/sending/sent/unknown/failed`, и неизвестный исход не запускает
  бесконечную рассылку (docs/01, раздел 6, пункт 6).
- Очистка сырого тела обновлений: срок удержания ≤24 часов после обработки,
  метаданные дедупликации живут дольше. Сейчас очистки нет вовсе.

Затем T-02c (action tokens для кнопок и завершение заданий через тот же
CommandBus), T-03 Mini App.
...
резервное копирование, установка вебхука у Telegram (`setWebhook` с секретом).
