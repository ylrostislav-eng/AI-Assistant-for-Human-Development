...
**T-02a — приём вебхука Telegram.** Первая часть бота: принять обновление и не
потерять его. Отвечать бот в этом срезе не должен; без надёжного приёма отвечать
нечем.

Что входит:

- `POST /telegram/webhook` — отдельная граница доверия, а не app bearer. Проверять
  `X-Telegram-Bot-Api-Secret-Token` (это **не** токен бота), размер тела и
  закрытую схему обновления.
- Запись в inbox с `UNIQUE (bot_id, update_id)` **до** ответа Telegram. Ответить
  успехом за несохранённое обновление нельзя: Telegram не повторяет то, что мы
  подтвердили, и сообщение пропадёт навсегда. При недоступной базе — не 200.
- Повтор уже обработанного обновления получает ack без второго эффекта.
- Отправитель берётся из подписанного поля обновления; `chat.id` его не заменяет.
  В пересланном сообщении это разные люди, и перепутать их значит выполнить
  чужую команду от имени владельца. Пересланное содержимое — данные, не
  полномочия автора.
- Retention сырого inbox ≤24 часов после обработки; метаданные дедупликации
  хранятся отдельно и дольше.

Контрольные случаи писать до реализации: подделанный и отсутствующий секрет,
повтор обновления, недоступная база, пересланное сообщение, чужой бот,
превышение размера. Секрет вебхука и токен бота не логировать.
...
команд. Не переносить расчёт XP в текст бота. Реальная частичная реализация
P1-01/02/03/06/07 не равна завершению фаз. Не заявлять core MVP по кнопке бота:
