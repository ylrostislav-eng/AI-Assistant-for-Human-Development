...
| T-04b-4 **следующая** | Runtime config и свободный текст бота **сделаны вне очереди** (handoff 3.21, 3.22); bounded live acceptance — нет | Base/protocol/model/key из secret/config, первичная/резервная модели явно выбраны; synthetic live tool cycle, отказ/timeout/лимит; `/new`, `/today`, кнопки работают без AI; policy и durable budget из предыдущих срезов обязательны |
