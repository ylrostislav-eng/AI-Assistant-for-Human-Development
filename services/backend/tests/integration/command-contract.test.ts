...
describe('R6: закрытые схемы нагрузки', () => {
  it('лишнее поле в нагрузке отклоняется, а не теряется молча', async () => {
    ...
    // Клиент присылает поле, которого в контракте нет. Молчаливый приём
    // означал бы, что человек считает данные записанными, а восстановить их
    // через неделю будет неоткуда.
    const response = await send(
      'complete_quest',
      { настроение: 'бодрое' },
      { aggregateId: occurrence.id, expectedVersion: occurrence.version },
